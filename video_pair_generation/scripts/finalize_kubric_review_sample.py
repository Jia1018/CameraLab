...
def update_progress(run_dir: Path, python_bin: Path, log_path: Path) -> dict[str, Any]:
    subprocess.run(
        [
            str(python_bin),
            str(PROJECT_ROOT / "video_pair_generation" / "scripts" / "update_kubric_progress.py"),
            "--run-dir",
            str(run_dir),
            "--python-bin",
            str(python_bin),
...
def main() -> None:
    ...
    export_cmd = [
        str(args.python_bin),
        str(PROJECT_ROOT / "video_pair_generation" / "scripts" / "export_kubric_review_sample.py"),
        "--source-run-dir",
    ...
    run_step(
        [
            ...
            str(PROJECT_ROOT / "video_pair_generation" / "scripts" / "make_run_previews.py"),
            "--run-dir",
            str(dest_run_dir),
        ],
        log_path,
    )
    run_step(["bash", str(PROJECT_ROOT / "video_pair_generation" / "scripts" / "sync_site_to_docs.sh")], log_path)
